...
D = os.path.expanduser("~/Desktop/ciclo-de-vida-ia")
SRC = os.path.join(D, sys.argv[1]) if len(sys.argv) > 1 else os.path.join(D, "ciclo-de-vida-desarrollo-asistido-por-ia.md")
...
# 1. extraer los bloques mermaid y renderizarlos a SVG
...
for i, b in enumerate(bloques, 1):
    ...
    if not os.path.exists(svg):
        ...
        if not os.path.exists(svg):
            print("fallo el diagrama %d:\n%s" % (i, r.stderr[-800:]), file=sys.stderr)
            sys.exit(1)
    s = io.open(svg, encoding="utf-8").read()
    s = re.sub(r"^<\?xml[^>]*\?>\s*", "", s)
    svgs.append(s)
    print("diagrama %d renderizado" % i)

# 2. sustituir los bloques por marcadores antes de convertir
def marcador(m, c=[0]):
    c[0] += 1
    return "\n\n@@DIAGRAMA%d@@\n\n" % c[0]
texto = re.sub(r"```mermaid\n.*?```", marcador, texto, flags=re.S)

cuerpo = markdown.markdown(
    texto, extensions=["tables", "fenced_code", "toc", "sane_lists", "attr_list"],
    extension_configs={"toc": {"title": "Contenido"}})

for i, s in enumerate(svgs, 1):
    cuerpo = cuerpo.replace("<p>@@DIAGRAMA%d@@</p>" % i,
                            '<div class="diagrama">%s</div>' % s)
...
.diagrama { break-inside: avoid; margin: 14pt 0 18pt; text-align: center; }
.diagrama svg { max-width: 100%; max-height: 225mm; height: auto; }
...
<style>%s</style></head><body>%s</body></html>""" % (CSS, cuerpo)
